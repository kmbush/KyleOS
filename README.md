...
<!-- Kyle: fill these two in before the final gate. [live site] = your URL (decide
     whether to hardcode your domain here — ADR-006 keeps it out of code/config, but
     the README front door is your call). [screenshot] = a real capture of the desktop. -->
...
flowchart LR
  ...
  subgraph read [Read path]
    B -->|GET| CF[CloudFront]
    CF -->|OAC| S3[(S3 · versioned)]
  end

  subgraph write [Write path · owner only]
    B -.#admin.-> COG[Cognito<br/>SRP + TOTP]
    COG -->|JWT| GW[API Gateway]
    GW --> L[Python Lambda]
    L -->|validate + write| S3
    L -->|invalidate| CF
  end
...
| Font | License |
|---|---|
| [Space Grotesk](https://github.com/floriankarsten/space-grotesk) | SIL Open Font License 1.1 — [`OFL.txt`](apps/web/public/fonts/space-grotesk/OFL.txt) |
| [JetBrains Mono](https://github.com/JetBrains/JetBrainsMono) | SIL Open Font License 1.1 — [`OFL.txt`](apps/web/public/fonts/jetbrains-mono/OFL.txt) |
